...
LlmConfig parse_config(std::string config_path) {
  ...
  config.embedding_file_ = llm_config.value("embedding_file", "embeddings_bf16.bin");
  ...
  config.kv_init_shape_ = llm_config.value("key_value_shape", std::vector<int>{});
  config.kv_init_shape_.insert(config.kv_init_shape_.begin(), config.layer_nums_);
...
device::Tensor* EmbeddingNode::gen_attention_mask(int seq_len, int all_seq_len, 
                                       base::DataType data_type,
                                       base::DataFormat data_format) {
    ...
    if(is_first_) {
        attention_mask = new device::Tensor(device, attention_mask_desc, "attention_mask");
    } else {
...
device::Tensor* EmbeddingNode::embedding(const std::vector<int32_t>& input_ids, 
                              int seq_len, int hidden_size, base::DataType data_type, 
                              base::DataFormat data_format,
...
base::Status EmbeddingNode::run() {
  ...
  auto inputs_embeds = embedding(token_ids[0], seq_len, hidden_size, 
                                 embedding_param->data_type_, 
                                 embedding_param->data_format_, embedding_file);

  auto attention_mask = gen_attention_mask(seq_len, all_seq_len, embedding_param->data_type_, 
                                           embedding_param->data_format_);

  auto position_ids = gen_position_ids(seq_len, all_seq_len,embedding_param->posid_data_type_, 
                                       embedding_param->data_format_);
...
base::Status SampleNode::run() {
  ...
  if(sample_params->is_prefill_) {
    tokenizer::TokenizerIds* token_ids = 
      (tokenizer::TokenizerIds*)(sample_params->token_ids_->getParam(sample_params->emb_node_));
  ...
  if(is_first_ && !sample_params->is_prefill_) {
    out_token->ids_.push_back({history_ids[history_ids.size()-1], out_token_id});
    is_first_ = false;
  } else {
...
int32_t SampleNode::sample(device::Tensor* logits, const std::vector<int>& history_ids) {
...
void LlmPrefillGraph::create_prefill_nodes_edges() {
    ...
    prefill_token_node_ = createNode<tokenizer::TokenizerCpp>("token_node", prompt_, prefill_token_ids_);
    ...
    std::vector<dag::Edge*> embedding_out = {prefill_input_ids_, prefill_attention_mask_, 
                                             prefill_position_ids_, prefill_past_key_values_};
    prefill_embedding_node_ = createNode<EmbeddingNode>("embedding_node", embedding_in, embedding_out);
    ...
    prefill_infer_node_ = createInfer<infer::Infer>("prefill_infer", 
                            inference_type_, embedding_out, infer_out);

    /* sample node */
    prefill_sample_node_ = createNode<SampleNode>("sample_node", 
                                    prefill_logits_, prefill_out_ids_);
}

void LlmPrefillGraph::set_params(bool is_path, 
  ...
  tokenizer::TokenizerPraram *token_param =
      dynamic_cast<tokenizer::TokenizerPraram*>(prefill_token_node_->getParam());
  token_param->is_encode_ = true;
  token_param->is_path_ = false;
  token_param->tokenizer_type_ = nndeploy::tokenizer::TokenizerType::kTokenizerTypeHF;
...
void LlmDecodeGraph::create_prefill_nodes_edges() {
    ...
    std::vector<dag::Edge*> embedding_in = {decode_embedding_ids_, decode_prefill_kv_};
    std::vector<dag::Edge*> embedding_out = {decode_input_ids_, decode_attention_mask_, 
                                             decode_position_ids_, decode_past_key_values_};
    ...
    decode_sample_node_ = createNode<SampleNode>("sample_node", 
                                    decode_logits_, decode_out_ids_);

    /* decode node */
    decode_node_ = createNode<tokenizer::TokenizerCpp>("decode_node", decode_out_ids_, 
                                                       decode_out_words_);
}

void LlmDecodeGraph::set_params(bool is_path, 
                                 base::ModelType model_type, 
                                 base::DeviceType device_type,
                                 LlmConfig& config) {
  /* embedding params */
...
base::Status LlmDecodeGraph::run() {
  ...
  for(int i=0; i<iters; ++i) {
    if(is_stop()) {
        tokenizer::TokenizerText* word = (tokenizer::TokenizerText*)(decode_out_words_->getParam(decode_node_));
    ...
    if(!is_first_) {
        ...
        embedding_param->all_seq_len_ = embedding_param->history_ids_.ids_[0].size();
        is_first_ = false;
    }
    ...
    tokenizer::TokenizerIds* token_id = 
            (tokenizer::TokenizerIds*)(decode_out_ids_->getParam(decode_sample_node_));
    *(embedding_param->past_kv_) = *(decode_presents_->getTensor(decode_infer_node_));
    ...
    bool not_append = std::find(sample_param->stop_tokens_.ids_[0].begin(), 
                            sample_param->stop_tokens_.ids_[0].end(), out_token_id) != 
                            sample_param->stop_tokens_.ids_[0].end();
    if(!not_append) 
        result_.append(word->texts_[0]);
  }
...
dag::Graph *createLlmLlama2Graph(
    ...
    base::DeviceType device_type, dag::Edge *prompt, dag::Edge *out,
  ...
  LlmPrefillGraph *prefill_graph = new LlmPrefillGraph("llama2_prefill", 
                                          prefill_in, prefill_out, 
                                          inference_type, 
                                          device_type, model_type, 
                                          is_path, config);
  llama2_graph->addNode(prefill_graph);
  std::vector<dag::Edge*> decode_out = {out};
  LlmDecodeGraph *decode_graph = new LlmDecodeGraph("llama2_decode", 
                                          prefill_out, decode_out, 
                                          prefill_graph->past_kv_,
                                          prefill_graph->history_ids_,
                                          inference_type, 
                                          device_type, model_type, 
                                          is_path, config);
