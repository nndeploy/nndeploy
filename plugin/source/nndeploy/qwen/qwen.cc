...
void LlmPrefillGraph::createPrefillNodesEdges() {
  // token node
  dag::NodeDesc prefill_token_desc("token_node", {"prompt"},
                                   {"prefill_token_ids"});
  this->setNodeDesc(prefill_token_node_, prefill_token_desc);

  // embedding node
  dag::NodeDesc prefill_embedding_desc(
      "embedding_node", {"prefill_token_ids"},
      {"prefill_input_ids", "prefill_attention_mask", "prefill_position_ids",
       "prefill_past_key_values"});
  this->setNodeDesc(prefill_embedding_node_, prefill_embedding_desc);

  // past kv
  genPastKeyValue();

  // prefill infer node
  dag::NodeDesc prefill_infer_desc(
      "prefill_infer",
      {"prefill_input_ids", "prefill_attention_mask", "prefill_position_ids",
       "prefill_past_key_values"},
      {"logits", "presents"});
  this->setNodeDesc(prefill_infer_node_, prefill_infer_desc);
  prefill_infer_node_->setInferenceType(inference_type_);

  // sample node
  dag::NodeDesc prefill_sample_desc("sample_node",
                                    {"logits", "prefill_token_ids"},
                                    {"prefill_out_ids", "history_ids"});
  this->setNodeDesc(prefill_sample_node_, prefill_sample_desc);
}
