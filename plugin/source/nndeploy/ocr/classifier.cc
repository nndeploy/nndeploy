...
base::Status ClassifierPostParam::serialize(rapidjson::Value &json,
...
base::Status ClassifierPreProcess::run() {
    ClassifierPreProcessParam *tmp_param = dynamic_cast<ClassifierPreProcessParam *>(param_.get());
...
base::Status ClassifierPostProcess::run() {
    ...
    for (int i = 0; i < batch; i++) {
        ...
        SingleBatchPostprocessor(data_ptr, length,
                                &cls_labels[i], &cls_scores[i]);
        if (cls_labels[i] % 2 == 1 &&
            cls_scores[i] > param->cls_thresh_) {
