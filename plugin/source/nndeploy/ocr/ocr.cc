...
// 使用 detector_results
    ...
    if (detector_result->boxes_.size() == 0) {
      ...
      boxes_recovered = util_post_processor_.FilterTagDetRes(boxes_recovered, {origin_w, origin_h, detector_result->detector_resized_w, detector_result->detector_resized_h});
      ...
      for (int i = 0; i < boxes_recovered.size(); i++) {
        ...
        detector_result->image_list_.emplace_back(ocr::GetRotateCropImage(*src, new_box));
    }
...
base::Status RotateImage180::run() {   
    ...
    for (int i = 0; i < crop_images_result->image_list_.size(); i++) {
      if (ocr_classifier_result->classifier_result[i] == 1) {
        cv::rotate(crop_images_result->image_list_[i], crop_images_result->image_list_[i], 1);
      }
    }
...
base::Status PrintOcrNodeParam::serialize(rapidjson::Value &json,
  ...
  json.AddMember("path_", rapidjson::Value(path_.c_str(), allocator), allocator);
  return base::kStatusCodeOk;
}
...
base::Status PrintOcrNode::run() {
  ...
  PrintOcrNodeParam *tmp_param = dynamic_cast<PrintOcrNodeParam *>(param_.get());
