...
base::Status DetectorPostParam::serialize(rapidjson::Value &json,
  ...
  json.AddMember("det_db_score_mode_", rapidjson::Value(det_db_score_mode_.c_str(), allocator), allocator);
...
base::Status DetectorPostParam::deserialize(rapidjson::Value &json) {
  ...
  if (json.HasMember("det_db_box_thresh_") && json["det_db_box_thresh_"].IsFloat()) {
    det_db_box_thresh_ = json["det_db_box_thresh_"].GetFloat();
  }
  if (json.HasMember("det_db_unclip_ratio_") && json["det_db_unclip_ratio_"].IsFloat()) {
    det_db_unclip_ratio_ = json["det_db_unclip_ratio_"].GetFloat();
  }
  if (json.HasMember("det_db_score_mode_") && json["det_db_score_mode_"].IsString()) {
...
base::Status DetectorPreProcess::run() {
    DetectorPreProcessParam *tmp_param = dynamic_cast<DetectorPreProcessParam *>(param_.get());
    ...
    if (desc.data_format_ == base::kDataFormatNCHW) {
        desc.shape_ = {1, preprocess::getChannelByPixelType(tmp_param->dst_pixel_type_), target_h, target_w};
    } else {
        desc.shape_ = {1, target_h, target_w, preprocess::getChannelByPixelType(tmp_param->dst_pixel_type_)};
    }
...
base::Status DetectorPostProcess::run() {
    ...
    boxes = util_post_processor_.BoxesFromBitmap(pred_map, bit_map, det_db_box_thresh, det_db_unclip_ratio, param->det_db_score_mode_);
