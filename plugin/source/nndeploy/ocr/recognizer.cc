...
base::Status RecognizerPostParam::serialize(rapidjson::Value &json,
  ...
  json.AddMember("character_path_", rapidjson::Value(character_path_.c_str(), allocator),
                   allocator);
...
void RecSingleBatchPostprocessor(const float* out_data,
                              ...
                              std::string* text, float* rec_score, std::vector<std::string> label_list) {
...
base::Status RecognizerPreProcess::run() {
    RecognizerPreProcessParam *tmp_param = dynamic_cast<RecognizerPreProcessParam *>(param_.get());
    ...
    for(size_t start_index = 0; start_index < detector_result->image_list_.size(); start_index+=rec_batch_size) {
        size_t end_index = std::min(start_index + rec_batch_size, detector_result->image_list_.size());
    ...
    for(size_t start_index = 0; start_index < detector_result->image_list_.size(); start_index+=rec_batch_size) {
        ...
        for (size_t i = 0; i< mats.size(); i++) {
            ...
            if (tmp_param->src_pixel_type_ != tmp_param->dst_pixel_type_) {
                ...
                int cv_cvt_type = preprocess::OpenCvConvert::convertFromCvtColorType(cvt_type);
            ...
            if (!static_shape_infer_) {
                ...
                if (tmp_param->interp_type_ != base::kInterpTypeNotSupport) {
                    int interp_type =
                        preprocess::OpenCvConvert::convertFromInterpType(tmp_param->interp_type_);
                    cv::resize(tmp_cvt, tmp_resize, cv::Size(resize_w, img_h_tmp), 0.0, 0.0,
                            interp_type);
                } else {
                    tmp_resize = tmp_cvt;
                }
                ...
                if (tmp_resize.cols < img_w_tmp) {
                    ...
                    preprocess::OpenCvUtil::copyMakeBorder(tmp_resize, tmp_pad, tmp_param->top_,
                ...
                preprocess::OpenCvConvert::convertToBatchTensor(tmp_pad, dst,
                                                true,         
                                                tmp_param->scale_,
                                                tmp_param->mean_,
...
base::Status RecognizerPostProcess::run() {
    ...
    YAML::Node config = YAML::LoadFile(character_path);

    // 访问 PostProcess -> character_dict
    YAML::Node dict = config["PostProcess"]["character_dict"];

    std::vector<std::string> label_list;
    if (dict.IsSequence()) {
        for (std::size_t i = 0; i < dict.size(); i++) {
            label_list.push_back(dict[i].as<std::string>());
        }
    }
    ...
    for (int i = 0; i < batch; i++) {
        ...
        RecSingleBatchPostprocessor(data_ptr, output_shape,
                                &texts[i], &rec_scores[i], label_list);
    }
