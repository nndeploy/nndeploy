 ...
 *
 * This class represents a fixed graph structure for image classification using
 * ResNet models. The graph consists of three main stages:
 * 1. Preprocessing: Converts input image format and resizes to model input size
 * 2. Inference: Runs the ResNet model inference
 * 3. Postprocessing: Processes model output to get classification results
 *
 * All node connections and data flow paths are determined at construction time
 * and cannot be modified during runtime. This provides better performance but
...
class ClassificationResnetGraph : public dag::Graph {
 public:
   ...
   * "data" edge name for model input Parameters control color space conversion
   ...
   *        inputs_: Must contain "resnetv17_dense0_fwd" matching model output
   ...
   * @note This allows runtime configuration of input format while maintaining
   * graph structure
   */
 ...
 * @details Dynamic graph containing three nodes: preprocessing, inference and
 * postprocessing Preprocessing node handles image format conversion and
 * resizing Inference node executes ResNet model inference Postprocessing node
...
class ClassificationResnetGraph : public dag::Graph {
 public:
   ...
   * format, size etc.
   * @param infer_param Inference parameters, including model path, device type
   * etc.
   * @param post_param Postprocessing parameters, including number of classes,
   * thresholds etc.
   */
  ClassificationResnetGraph(const std::string &name,
                            preprocess::CvtColorResizeParam *pre_param,
   ...
   * @param output_names List of specified output names for intermediate results
   ...
   * 3. Postprocessing: Process model output to get final classification results
   */
  std::vector<dag::Edge *> forward(
      std::vector<dag::Edge *> inputs,
      std::initializer_list<const std::string &> output_names = {}) {
