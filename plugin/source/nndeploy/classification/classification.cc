...
base::Status ClassificationPostProcess::run() {
  ...
  std::shared_ptr<ir::SoftmaxParam> op_param = std::make_shared<ir::SoftmaxParam>();
  op_param->axis_ = 1;
...
dag::Graph *createClassificationResnetGraph(
  ...
  dag::Node *post =
      graph->createNode<ClassificationPostProcess>("postprocess", infer_output, output);
  ...
  ClassificationPostParam *post_param = dynamic_cast<ClassificationPostParam *>(post->getParam());
  post_param->topk_ = 1;
