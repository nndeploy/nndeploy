...
class NNDEPLOY_CC_API SAMPointsParam : public base::Param {
 public:
  ...
  std::vector<float> box_;  // [x1, y1, x2, y2] for box prompt
  int ori_width;
  int ori_height;
  int version_ = -1;
  bool use_box_ = false;  // encode box as SAM box prompt (labels 2/3)
...
class NNDEPLOY_CC_API SAMGraph : public dag::Graph {
 public:
  ...
  std::vector<dag::Edge*> forward(std::vector<dag::Edge*> inputs) {
    ...
    std::vector<dag::Edge*> encoder_reshaped =
        (*encoder_input_reshape_node_)(encoder_input);
    std::vector<dag::Edge*> image_embedding =
        (*encoder_infer_node_)(encoder_reshaped);
 ...
 private:
  ...
  dag::Node* encoder_input_reshape_node_ = nullptr;
