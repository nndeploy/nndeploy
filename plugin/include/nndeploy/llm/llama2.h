...
class NNDEPLOY_CC_API EmbeddingNode: public dag::Node {
 ...
 protected:
  device::Tensor* embedding(const std::vector<int32_t>& input_ids, 
...
class NNDEPLOY_CC_API SampleNode: public dag::Node {
 public:
  SampleNode(const std::string &name, dag::Edge* &input,
             dag::Edge* &output)
...
class NNDEPLOY_CC_API PromptNode: public dag::Node {
 public:
  PromptNode(const std::string &name, dag::Edge *input,
             dag::Edge *output)
...
class NNDEPLOY_CC_API LlmPrefillGraph: public dag::Graph {
 public:
  ...
  LlmPrefillGraph(const std::string &name, 
            ...
            LlmConfig& config)
    : dag::Graph(name, inputs, outputs), prompt_(inputs[0]), 
      prefill_presents_(outputs[1]), prefill_out_ids_(outputs[0]), 
      inference_type_(inference_type), hidden_size_(config.hidden_size_),
...
class NNDEPLOY_CC_API LlmDecodeGraph: public dag::Loop {
 ...
 protected:
  ...
  inline bool is_stop() {
    ...
    if(is_first_) {
      token_ids = (tokenizer::TokenizerIds*)(decode_embedding_ids_->getParam(decode_embedding_node_));
    } else {
      token_ids = (tokenizer::TokenizerIds*)(decode_out_ids_->getParam(decode_sample_node_));
    }
