 ...
 public:
  ...
  virtual base::Status run() {
    ...
    ocr::OCRResult *result =
        (ocr::OCRResult *)inputs_[1]->get<OCRResult>(this);
    if (result == nullptr) {
    ...
    boxes_recovered = util_post_processor_.FilterTagDetRes(boxes_recovered, {origin_w, origin_h, result->detector_resized_w, result->detector_resized_h}); // 如果需要原图缩放信息可以传入
