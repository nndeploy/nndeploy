#ifndef _NNDEPLOY_DETECT_RECOGNIZER_OCR_OCR_H_
#define _NNDEPLOY_DETECT_RECOGNIZER_OCR_OCR_H_
...
class NNDEPLOY_CC_API RecognizerPreProcessParam : public base::Param {
 public:
  ...
  virtual base::Status serialize(
    ...
    json.AddMember("rec_batch_size_", rec_batch_size_, allocator);
    rapidjson::Value rec_image_shape(rapidjson::kArrayType);
    for (int i = 0; i < 3; i++) {
      rec_image_shape.PushBack(rec_image_shape_[i], allocator);
    }
    json.AddMember("rec_image_shape_", rec_image_shape, allocator);
...
class NNDEPLOY_CC_API RecognizerPreProcess : public dag::Node {
 public:
...
class NNDEPLOY_CC_API RecognizerPostProcess : public dag::Node {
 public:
  RecognizerPostProcess(const std::string &name) : dag::Node(name) {
    key_ = "nndeploy::ocr::RecognizerPostProcess";
    desc_ = "PPOcrRecv3/v4/v5 postprocess[device::Tensor->DetectResult]";
    param_ = std::make_shared<RecognizerPostParam>();
...
class NNDEPLOY_CC_API RecognizerGraph : public dag::Graph {
 public:
  ...
  virtual base::Status defaultParam() {
    RecognizerPreProcessParam *pre_param =
        dynamic_cast<RecognizerPreProcessParam *>(pre_->getParam());
    pre_param->src_pixel_type_ = base::kPixelTypeBGR;
    pre_param->dst_pixel_type_ = base::kPixelTypeBGR;
    pre_param->interp_type_ = base::kInterpTypeLinear;
    pre_param->rec_batch_size_ = 6;
    pre_param->rec_image_shape_ = {3, 48, 320};
    RecognizerPostParam *post_param =
        dynamic_cast<RecognizerPostParam *>(post_->getParam());
    post_param->rec_thresh_ = 0.6;
  ...
  base::Status setSrcPixelType(base::PixelType pixel_type) {
    RecognizerPreProcessParam *param =
        dynamic_cast<RecognizerPreProcessParam *>(pre_->getParam());
