...
#ifndef _NNDEPLOY_DETECT_DETECT_YOLO_YOLO_NAS_H_
#define _NNDEPLOY_DETECT_DETECT_YOLO_YOLO_NAS_H_
...
class NNDEPLOY_CC_API YoloNasPostParam : public base::Param {
 public:
...
class NNDEPLOY_CC_API YoloNasPostProcess : public dag::Node {
 public:
  YoloNasPostProcess(const std::string& name) : dag::Node(name) {
    key_ = "nndeploy::detect::YoloNasPostProcess";
    desc_ = "YOLO-NAS postprocess[device::Tensor->BBoxResult]";
    param_ = std::make_shared<YoloNasPostParam>();
    this->setInputTypeInfo<device::Tensor>();
    this->setInputTypeInfo<device::Tensor>();
    this->setOutputTypeInfo<BBoxResult>();
  }
  YoloNasPostProcess(const std::string& name, std::vector<dag::Edge*> inputs,
  ...
  virtual ~YoloNasPostProcess() {}

  virtual base::Status run();
};

class NNDEPLOY_CC_API YoloNasGraph : public dag::Graph {
 public:
  YoloNasGraph(const std::string& name) : dag::Graph(name) {
    key_ = "nndeploy::detect::YoloNasGraph";
    desc_ =
        "YOLO-NAS graph[cv::Mat->preprocess->infer->postprocess->BBoxResult]";
    ...
    post_ = dynamic_cast<YoloNasPostProcess*>(
        this->createNode<YoloNasPostProcess>("postprocess"));
  }

  YoloNasGraph(const std::string& name, std::vector<dag::Edge*> inputs,
               std::vector<dag::Edge*> outputs)
  ...
  virtual base::Status defaultParam() {
    ...
    YoloNasPostParam* post_param =
        dynamic_cast<YoloNasPostParam*>(post_->getParam());
...
#endif /* _NNDEPLOY_DETECT_DETECT_YOLO_YOLO_NAS_H_ */
