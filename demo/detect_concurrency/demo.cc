...
int main(int argc, char *argv[]) {
  ...
  // 有向无环图pipeline的输入边packert
  model::Packet input1("detect_in");
  // 有向无环图pipeline的输出边packert
  model::Packet output1("detect_out");
  // 创建检测模型有向无环图pipeline
  model::Pipeline *pipeline1 =
      model::createPipeline(name, inference_type, device_type, &input1,
                            &output1, model_type, is_path, model_value);
  if (pipeline == nullptr) {
  ...
  std::mutex tt;
  int i = 0;
  auto funcs = [&](model::Pipeline *pipeline) {
