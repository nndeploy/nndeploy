...
int main(int argc, char *argv[]) {
  ...
  // 模型路径或者模型字符串
  std::vector<std::string> classifier_model_value = demo::getClassifierModelValue();
  std::vector<std::string> recognizer_model_value = demo::getRecognizerModelValue();
  ...
  std::vector<std::string> classifier_model_inputs = demo::getClassifierModelInputs();
  std::vector<std::string> recognizer_model_inputs = demo::getRecognizerModelInputs();
  std::vector<std::string> detector_model_inputs = demo::getDetectorModelInputs();
  std::vector<std::string> classifier_model_outputs = demo::getClassifierModelOutputs();
  std::vector<std::string> recognizer_model_outputs = demo::getRecognizerModelOutputs();
  std::vector<std::string> detector_model_outputs = demo::getDetectorModelOutputs();
  ...
  dag::NodeDesc infer_desc("infer", detector_model_inputs, detector_model_outputs);
  dag::NodeDesc post_desc("postprocess", detector_model_outputs, {"detector_output"});
  v_graph->make(pre_desc, infer_desc, inference_type, post_desc);
  v_graph->setInferParam(device_type, model_type, is_path, detector_model_value);
  ...
  dag::Edge *classifier_output = graph->createEdge("classifier_output"); // 最终输出
  ...
  classify_graph = new ocr::ClassifierGraph("Classifier", {classifier_input}, {classifier_output});
  auto *c_graph = dynamic_cast<ocr::ClassifierGraph *>(classify_graph);
  dag::NodeDesc c_pre_desc("c_preprocess", {"rotate_crop_output"}, classifier_model_inputs);
  dag::NodeDesc c_infer_desc("c_infer", classifier_model_inputs, classifier_model_outputs);
  dag::NodeDesc c_post_desc("c_postprocess", classifier_model_outputs, {"classifier_output"});
  c_graph->make(c_pre_desc, c_infer_desc, inference_type, c_post_desc);
  c_graph->setInferParam(device_type, model_type, is_path, classifier_model_value);
  // v_graph->setVersion(version);
  ...
  rotate_180_node = graph->createNode<ocr::RotateImage180>(
        "RotateImage180", {rotate_180_input, rotate_crop_output}, {rotate_180_output});
  ...
  recognizer_graph = new ocr::RecognizerGraph("Recognizer", {recognizer_input}, {recognizer_output});
  auto *r_graph = dynamic_cast<ocr::RecognizerGraph *>(recognizer_graph);
  dag::NodeDesc r_pre_desc("r_preprocess", {"rotate_180_output"}, recognizer_model_inputs);
  dag::NodeDesc r_infer_desc("r_infer", recognizer_model_inputs, recognizer_model_outputs);
  dag::NodeDesc r_post_desc("r_postprocess", recognizer_model_outputs, {"detect_out"});
  r_graph->make(r_pre_desc, r_infer_desc, inference_type, r_post_desc);
  r_graph->setInferParam(device_type, model_type, is_path, recognizer_model_value);
  ...
  auto* print_node = dynamic_cast<ocr::PrintOcrNode*>(graph->createNode<ocr::PrintOcrNode>(
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (pt != base::kParallelTypePipeline) {
      ocr::OCRResult *result =
          (ocr::OCRResult *)output->getGraphOutputParam();
      if (result == nullptr) {
