...
base::Status OpSign::run() {
  ...
  for (long i = 0; i < input_elements; ++i) {
    output_data[i] = (input_data[i] > 0)   ? 1.0f
                     : (input_data[i] < 0) ? -1.0f
                                           : 0.0f;
  }
...
base::Status sign(device::Tensor* input, device::Tensor* output) {
  ...
  Op* op = createOp(input->getDeviceType(), "", ir::kOpTypeSign);
  if (op == nullptr) {
...
REGISTER_OP_IMPLEMENTION(kDeviceTypeCodeCpu, ir::kOpTypeSign, OpSign)
