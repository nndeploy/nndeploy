...
base::Status OpCosh::run() {
  ...
  for (long i = 0; i < input_elements; ++i) {
    output_data[i] = std::cosh(input_data[i]);
  }
...
base::Status cosh(device::Tensor* input, device::Tensor* output) {
  ...
  Op* op = createOp(input->getDeviceType(), "", ir::kOpTypeCosh);
  if (op == nullptr) {
...
REGISTER_OP_IMPLEMENTION(kDeviceTypeCodeCpu, ir::kOpTypeCosh, OpCosh)
