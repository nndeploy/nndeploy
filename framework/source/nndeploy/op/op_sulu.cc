...
base::Status OpSelu::run() {
  ...
  const float alpha = 1.67326f, scale = 1.0507f;
  for (long i = 0; i < input_elements; ++i) {
    output_data[i] = (input_data[i] > 0)
                         ? scale * input_data[i]
                         : scale * (alpha * (std::exp(input_data[i]) - 1));
  }
...
base::Status selu(device::Tensor* input, device::Tensor* output) {
  ...
  Op* op = createOp(input->getDeviceType(), "", ir::kOpTypeSelu);
  if (op == nullptr) {
...
REGISTER_OP_IMPLEMENTION(kDeviceTypeCodeCpu, ir::kOpTypeSelu, OpSelu)
