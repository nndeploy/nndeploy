...
base::Status OpHardSigmoid::run() {
  ...
  for (long i = 0; i < input_elements; ++i) {
    output_data[i] =
        std::max(0.0f, std::min(1.0f, (input_data[i] + 3.0f) / 6.0f));
  }
...
base::Status hardsigmoid(device::Tensor* input, device::Tensor* output) {
  ...
  Op* op = createOp(input->getDeviceType(), "", ir::kOpTypeHardSigmoid);
  if (op == nullptr) {
...
REGISTER_OP_IMPLEMENTION(kDeviceTypeCodeCpu, ir::kOpTypeHardSigmoid,
                         OpHardSigmoid)
