...
// Gelu 算子的 X86 (oneDNN) 实现
class X86OpGelu : public OpGelu {
 public:
  X86OpGelu() : OpGelu() {}
  virtual ~X86OpGelu() {}

  virtual base::Status init() {
    base::Status status = OpGelu::init();
  ...
  virtual base::Status preRun() {
    ...
    gelu_src_mem_ = dnnl::memory(src_md, dnnl_engine_, inputs_[0]->getData());
    gelu_dst_mem_ = dnnl::memory(src_md, dnnl_engine_, outputs_[0]->getData());

    dnnl_gelu_pd_ = dnnl::eltwise_forward::primitive_desc(dnnl_engine_,
            dnnl::prop_kind::forward_inference, dnnl::algorithm::eltwise_gelu_erf, src_md, dst_md, 0.f, 0.f);

	  return base::kStatusCodeOk;
  }

  virtual base::Status run() {
    auto gelu_e = dnnl::eltwise_forward(dnnl_gelu_pd_);
    gelu_e.execute(dnnl_stream_,
                    {{DNNL_ARG_SRC, gelu_src_mem_}, {DNNL_ARG_DST, gelu_dst_mem_}});
 ...
 private:
  ...
  dnnl::eltwise_forward::primitive_desc dnnl_gelu_pd_;
  dnnl::memory gelu_src_mem_;
  dnnl::memory gelu_dst_mem_;
};

// 注册 OpGelu 算子
REGISTER_OP_IMPLEMENTION(kDeviceTypeCodeX86, ir::kOpTypeGelu, X86OpGelu)
