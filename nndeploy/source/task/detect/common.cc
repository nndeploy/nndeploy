...
#include "nndeploy/source/task/detect/common.h"
...
void FilterScoresAndTopk(const Tensor& scores, float score_thr, int topk,
...
float iOU(float xmin0, float ymin0, float xmax0, float ymax0, float xmin1,
  ...
  auto area = w * h;
  auto sum =
      (xmax0 - xmin0) * (ymax0 - ymin0) + (xmax1 - xmin1) * (ymax1 - ymin1);
...
void Sort(std::vector<float>& probs, std::vector<int>& label_ids,
          std::vector<int>& anchor_idxs) {
