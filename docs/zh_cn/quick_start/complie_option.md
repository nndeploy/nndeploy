...
| ENABLE_NNDEPLOY_OPENMP |  是否使用OpenMP | ARM / Linux / X86 | ON |
| ENABLE_NNDEPLOY_ADDRESS_SANTIZER |  是否开启内存泄漏检测 | OFF |
| ENABLE_NNDEPLOY_TIME_PROFILER | 是否开启时间性能Profile | ON |
| ENABLE_NNDEPLOY_OPENCV |  是否链接第三方库[opencv]((https://github.com/opencv/opencv))（适用于ARM / Linux / X86平台） | OFF |
...
| ENABLE_NNDEPLOY_DEVICE_ARM |   是否使能device arm （适应于ARM / Linux平台） | OFF |
...
| ENABLE_NNDEPLOY_DEVICE_HVX | 是否使能device apple hvx （适应于高通和联发科） |OFF |
| ENABLE_NNDEPLOY_DEVICE_MTK_VPU | 是否使能device apple hvx（适应于高通和联发科） |OFF |
| ENABLE_NNDEPLOY_DEVICE_ASCEND_CL |  是否使能device apple ascend cl（适应于高通和联发科） |ON |
...
| ENABLE_NNDEPLOY_INFERENCE_ASCEND_CL |   是否使能INFERENCE ASCEND_CL | OFF |
