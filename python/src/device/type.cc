...
NNDEPLOY_API_PYBIND11_MODULE("device", m) {
  ...
  py::class_<BufferDesc>(m, "BufferDesc")
      ...
      // .def("justModify",
      //      py::overload_cast<const base::SizeVector
      //      &>(&BufferDesc::justModify))
      // .def("justModify",
  ...
  py::class_<TensorDesc>(m, "TensorDesc")
      ...
      .def("serialize", &TensorDesc::serialize, py::arg("stream"))
      .def("deserialize", &TensorDesc::deserialize, py::arg("stream"))
      .def("print", &TensorDesc::print, py::arg("stream") = std::cout)
