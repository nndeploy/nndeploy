...
NNDEPLOY_API_PYBIND11_MODULE("ocr", m) {
  py::class_<DetectorPreProcessParam, base::Param, std::shared_ptr<DetectorPreProcessParam>>(
      m, "DetectorPreProcessParam")
      .def(py::init<>())
      .def_readwrite("src_pixel_type_", &DetectorPreProcessParam::src_pixel_type_)
      .def_readwrite("dst_pixel_type_", &DetectorPreProcessParam::dst_pixel_type_)
      ...
      .def_property(
          ...
          [](const DetectorPreProcessParam& self) {
            return py::array_t<float>({3}, {sizeof(float)}, self.scale_, py::cast(self));
          },
          [](DetectorPreProcessParam& self, py::array_t<float> arr) {
            auto buf = arr.request();
            if (buf.ndim != 1 || buf.shape[0] > 4) {
              throw std::runtime_error("Input array must be 1D with 3 elements");
            }
      ...
      .def_property(
          "mean_",
          [](const DetectorPreProcessParam& self) {
            return py::array_t<float>({3}, {sizeof(float)}, self.mean_, py::cast(self));
          },
          [](DetectorPreProcessParam& self, py::array_t<float> arr) {
            auto buf = arr.request();
            if (buf.ndim != 1 || buf.shape[0] > 4) {
      ...
      .def_property(
          "std_",
          [](const DetectorPreProcessParam& self) {
            return py::array_t<float>({3}, {sizeof(float)}, self.std_, py::cast(self));
          },
          [](DetectorPreProcessParam& self, py::array_t<float> arr) {
            auto buf = arr.request();
            if (buf.ndim != 1 || buf.shape[0] > 4) {
      ...
      .def_property(
          "border_val_",
          [](const DetectorPreProcessParam& self) {
            return py::array_t<double>({3}, {sizeof(double)}, self.border_val_.val_, py::cast(self));
          },
  ...
  py::class_<DetectorPostParam, base::Param, std::shared_ptr<DetectorPostParam>>(
      m, "DetectorPostParam")
      .def(py::init<>())
      .def_readwrite("det_db_thresh_", &DetectorPostParam::det_db_thresh_)  // 分数阈值，用于决定哪些检测框被保留
      .def_readwrite("det_db_box_thresh_", &DetectorPostParam::det_db_box_thresh_)  // 非最大抑制(NMS)阈值，用于合并重叠的检测框 
      .def_readwrite("det_db_unclip_ratio_", &DetectorPostParam::det_db_unclip_ratio_)  // 模型可以识别的类别数量
      .def_readwrite("det_db_score_mode_", &DetectorPostParam::det_db_score_mode_)      // 模型输入图像的高度
  ...
  py::class_<ClassifierPreProcessParam, base::Param, std::shared_ptr<ClassifierPreProcessParam>>(
      m, "ClassifierPreProcessParam")
      .def(py::init<>())
      .def_readwrite("src_pixel_type_", &ClassifierPreProcessParam::src_pixel_type_)
      .def_readwrite("dst_pixel_type_", &ClassifierPreProcessParam::dst_pixel_type_)
    ...
    [](ClassifierPreProcessParam &self, py::array_t<int> arr) {
        ...
        if (buf.ndim != 1 || buf.shape[0] != 3) {
            throw std::runtime_error("Input array must be 1D with exactly 3 elements");
        }
  ...
  py::class_<ClassifierPostParam, base::Param, std::shared_ptr<ClassifierPostParam>>(
      m, "ClassifierPostParam")
      .def(py::init<>())
  ...
  py::class_<RecognizerPreProcessParam, base::Param, std::shared_ptr<RecognizerPreProcessParam>>(
      m, "RecognizerPreProcessParam")
      .def(py::init<>())
      .def_readwrite("src_pixel_type_", &RecognizerPreProcessParam::src_pixel_type_)
      .def_readwrite("dst_pixel_type_", &RecognizerPreProcessParam::dst_pixel_type_)
      ...
      .def_readwrite("rec_batch_size_", &RecognizerPreProcessParam::rec_batch_size_)
      ...
      .def_readwrite("right_", &RecognizerPreProcessParam::right_)
        ...
        [](const RecognizerPreProcessParam &self) {
            return py::array_t<int>(
                {self.rec_image_shape_.size()},   
                {sizeof(int)},                    
                self.rec_image_shape_.data(),     
                py::cast(&self)                   
            );
        },
  ...
  py::class_<RecognizerPostParam, base::Param, std::shared_ptr<RecognizerPostParam>>(
      m, "RecognizerPostParam")
      .def(py::init<>())
  ...
  py::class_<RecognizerGraph, dag::Graph>(m, "RecognizerGraph")
    ...
    .def_property(
        ...
        [](OCRResult &self) {
            ...
            for (auto &img : self.image_list_) {
                py_list.append(img);  // 需要先绑定 cv::Mat -> numpy (cv::Mat 转换)
            }
            return py_list;
        },
        [](OCRResult &self, py::list list) {
    ...
    .def_property(
        "text_",
        [](OCRResult &self) { return py::cast(self.text_); },
        [](OCRResult &self, py::list list) {
            self.text_.clear();
            for (auto item : list) {
    ...
    .def_property(
        "table_html_",
        [](OCRResult &self) { return self.table_html_; },
        [](OCRResult &self, const std::string &val) { self.table_html_ = val; })
    ...
    py::class_<RotateCropImage, dag::Node>(
      m, "RotateCropImage")
      .def(py::init<const std::string &>())
      .def(py::init<const std::string &, std::vector<dag::Edge *>,
                    std::vector<dag::Edge *>>())
    ...
    py::class_<RotateImage180, dag::Node>(
      m, "RotateImage180")
      .def(py::init<const std::string &>())
      .def(py::init<const std::string &, std::vector<dag::Edge *>,
                    std::vector<dag::Edge *>>())
    ...
    py::class_<DrawDetectorBox, dag::Node>(
      m, "DrawDetectorBox")
      .def(py::init<const std::string &>())
      .def(py::init<const std::string &, std::vector<dag::Edge *>,
                    std::vector<dag::Edge *>>())
    ...
    .def_property(
        "texts_",
        [](OcrText &self) { return py::cast(self.texts_); },
    ...
    py::class_<PrintOcrNodeParam, base::Param, std::shared_ptr<PrintOcrNodeParam>>(
      m, "PrintOcrNodeParam")
      .def(py::init<>())
      .def_readwrite("path_", &PrintOcrNodeParam::path_);

    py::class_<PrintOcrNode, dag::Node>(
      m, "PrintOcrNode")
      .def(py::init<const std::string &, std::vector<dag::Edge *>,
                    std::vector<dag::Edge *>>())
