...
class TestSupportedUnaryOperators(unittest.TestCase):
    ...
    def _test_operator(
        self, nndeploy_func, torch_func, cpu_src, rtol=1e-02, atol=1e-04
    ):
        ...
        nndeploy_result = nndeploy_func(cpu_src)
        nndeploy_result = create_numpy_from_tensor(nndeploy_result)
    ...
    def test_acos(self):
        """测试Acos（支持，输入范围[-1,1]）"""
        self._test_operator(
            nndeploy_func=F.acos, torch_func=torch.acos, cpu_src=self.cpu_range_11_src
        )

    def test_asin(self):
        """测试Asin（支持，输入范围[-1,1]）"""
        self._test_operator(
            nndeploy_func=F.asin, torch_func=torch.asin, cpu_src=self.cpu_range_11_src
        )
    ...
    def test_log(self):
        """测试Log（支持，输入为正数）"""
        self._test_operator(
            nndeploy_func=F.log, torch_func=torch.log, cpu_src=self.cpu_positive_src
        )
    ...
    def test_tan(self):
        ...
        self._test_operator(
            nndeploy_func=F.tan, torch_func=torch.tan, cpu_src=cpu_safe_tan_src
        )
