...
class FaceSwapperDemo(nndeploy.dag.Graph):
    ...
    def forward(self, inputs: [nndeploy.dag.Edge] = []):
        decodec_source_outputs = self.decodec_source(inputs)
        decodec_target_outputs = self.decodec_target(inputs)
...
def test_face_swapper():    
    ...
    output = face_swapper_demo.trace(inputs)
    ...
    face_swapper_demo.set_output_path("/home/always/github/Deep-Live-Cam/nndeploy_Always_Chunel.jpg")
    nndeploy.base.time_point_start("face_swapper_demo_python")
    for i in range(count):
        print(f"test_face_swapper {i}")
        output = face_swapper_demo(inputs)
